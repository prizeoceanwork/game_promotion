...
export default function AdminSocialBuildingGroup() {
  ...
  useEffect(() => {
    if (authError && !authLoading) {
      setLocation("/socialbuildinggroup/login");
    }
  ...
  const {
    ...
    queryKey: ["/api/socialbuildinggroup/admin/registrations"],
    queryFn: async () => {
      const response = await apiRequest("GET", "/api/socialbuildinggroup/admin/registrations");
      if (!response.ok) {
  ...
  const deleteRegistration = useMutation({
    mutationFn: async (id: string) => {
     const response = await apiRequest("DELETE", `/api/socialbuildinggroup/admin/registrations/${id}`);
      if (!response.ok) {
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/socialbuildinggroup/admin/registrations"] });
      toast({
        title: "Success",
  ...
  const bulkDeleteMutation = useMutation({
    mutationFn: async (ids: string []) => {
       const response = await apiRequest("POST", "/api/socialbuildinggroup/admin/registrations/bulk-delete", { ids });
      if (!response.ok) {
  ...
  const { data: settings } = useQuery({
    ...
    queryFn: async () => {
      const response = await apiRequest("GET", "/api/appliancegallery/admin/settings");
      if (!response.ok) {
  ...
  const updateSettingMutation = useMutation({
    mutationFn: async ({ key, value }: { key: string; value: string }) => {
     const response = await apiRequest("PUT", `/api/appliancegallery/admin/settings/${key}`, { value });
      if (!response.ok) {
        const error = await response.json();
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/appliancegallery/admin/settings"] });
      toast({
        title: "Success",
    ...
    <div className="flex flex-col md:flex-row justify-between items-start md:items-center py-4 space-y-4 md:space-y-0">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:space-x-3 space-y-2 sm:space-y-0 w-full md:w-auto">
        ...
        <div className="flex flex-col sm:flex-row w-full sm:w-auto space-y-2 sm:space-y-0 sm:space-x-2">
          ...
          <Button
            onClick={() => queryClient.invalidateQueries({ queryKey: ["/api/appliancegallery/admin/registrations"] })}
            variant="outline"
            size="sm"
            className="w-full sm:w-auto bg-white/10 hover:bg-white/20 border-white/30 text-white hover:text-white backdrop-blur-sm"
          >
