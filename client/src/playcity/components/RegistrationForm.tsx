...
export default function RegistrationForm({
  ...
  const { data: videoRequirementSetting } = useQuery({
    queryKey: ["/api/playcity/settings/video_requirement_enabled"],
    queryFn: async () => {
      const response = await apiRequest("GET", "/api/playcity/settings/video_requirement_enabled");
  ...
  const registerMutation = useMutation({
    mutationFn: async (data: FormData) => {
      const response = await apiRequest("POST", "/api/playcity/register", {
        ...data,
        videoWatched,
      });
      return response.json();
    ...
    onSuccess: (variables) => {
      ...
      queryClient.invalidateQueries({ queryKey: ["/api/playcity/stats"] });
      form.reset();
      // Redirect to game page after a short delay
      setTimeout(() => {
        setLocation("/playcity/game");
      }, 1500);
    },
      ...
      <div className="container mx-auto px-4 relative z-10">
        <div className="max-w-2xl mx-auto">
          <div className="text-center mb-12">
            <div className="inline-block p-4 bg-[linear-gradient(to_bottom_right,_#4AC9F6_0%,_#1E90FF_25%,_#F7902B_55%,_#FF6600_100%)] rounded-full mb-8 shadow-xl">
              <Trophy className="text-white" size={40} />
            </div>
            <h2
              className="text-3xl md:text-4xl font-black text-blue-800  mb-4"
          ...
          {/* Registration Form */}
          <div className="bg-[linear-gradient(to_bottom_right,_#4AC9F6_0%,_#1E90FF_25%,_#F7902B_55%,_#FF6600_100%)] rounded-2xl shadow-2xl p-2">
            <div className="bg-white rounded-xl p-8 shadow-inner relative">
              {/* Play Now badge */}
              <div className="absolute -top-4 left-1/2 transform -translate-x-1/2 bg-[linear-gradient(to_right,_#4AC9F6_0%,__#FF6600_100%)]   text-white px-6 py-2 rounded-full shadow-lg">
                <span
              ...
              <Form {...form}>
                ...
                >
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel
                          ...
                          style={{ fontFamily: "Montserrat, sans-serif" }}
                        >
                          <User className="mr-2 text-blue-800 " size={16} />
                          First Name
                        </FormLabel>
                        <div className="relative">
                          <FormControl>
                            <Input
                              ...
                              className={`pr-10 ${(videoRequirementEnabled && !videoWatched) ? "bg-blue-200 cursor-not-allowed" : "bg-white"}`}
                              style={{ fontFamily: "Montserrat, sans-serif" }}
                            />
                          </FormControl>
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel
                          ...
                          style={{ fontFamily: "Montserrat, sans-serif" }}
                        >
                          <Phone className="mr-2 text-blue-800 " size={16} />
                          Phone Number
                        </FormLabel>
                        <div className="relative">
                          <FormControl>
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel
                          ...
                          style={{ fontFamily: "Montserrat, sans-serif" }}
                        >
                          <Mail className="mr-2 text-blue-800 " size={16} />
                          Email Address
                        </FormLabel>
                        <div className="relative">
                          <FormControl>
                            <Input
                              ...
                              type="email"
                              disabled={videoRequirementEnabled && !videoWatched}
                              placeholder="playcity@gmail.com"
                              className={`pr-10 ${(videoRequirementEnabled && !videoWatched) ? "bg-blue-200 cursor-not-allowed" : "bg-white"}`}
                              style={{ fontFamily: "Montserrat, sans-serif" }}
                            />
                          </FormControl>
                  ...
                  <div className="relative">
                    {(!videoRequirementEnabled || videoWatched) && (
                      <div className="absolute -inset-1 bg-[linear-gradient(to_bottom_right,_#4AC9F6_0%,_#1E90FF_25%,_#F7902B_55%,_#FF6600_100%)] rounded-lg blur opacity-30"></div>
                    )}
                    <Button
                      type="submit"
                      disabled={(videoRequirementEnabled && !videoWatched) || registerMutation.isPending}
                      className={`relative w-full bg-[linear-gradient(to_right,_#4AC9F6_0%,_#1E90FF_25%,_#F7902B_55%,_#FF6600_100%)]  text-white font-black py-4 sm:py-5 md:py-6 px-4 sm:px-6 md:px-8 rounded-lg transform hover:scale-105 transition-all duration-300 shadow-2xl text-lg ${
                        (videoRequirementEnabled && !videoWatched)
