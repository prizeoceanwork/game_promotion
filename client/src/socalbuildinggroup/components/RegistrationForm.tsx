...
export default function RegistrationForm({
  ...
  const { data: videoRequirementSetting } = useQuery({
    queryKey: ["/api/socal/settings/video_requirement_enabled"],
    queryFn: async () => {
      const response = await apiRequest("GET", "/api/socal/settings/video_requirement_enabled");
  ...
  const registerMutation = useMutation({
    mutationFn: async (data: FormData) => {
      const response = await apiRequest("POST", "/api/socal/register", {
        ...data,
        videoWatched,
      });
      return response.json();
    ...
    onSuccess: (variables) => {
      ...
      queryClient.invalidateQueries({ queryKey: ["/api/socal/stats"] });
      form.reset();
      // Redirect to game page after a short delay
      setTimeout(() => {
        setLocation("/socal/game");
      }, 1500);
    },
                        ...
                        <div className="relative">
                          <FormControl>
                            <Input
                              ...
                              type="email"
                              disabled={videoRequirementEnabled && !videoWatched}
                              placeholder="socalbuildinggroup@gmail.com"
                              className={`pr-10 ${(videoRequirementEnabled && !videoWatched) ? "bg-gray-200 cursor-not-allowed" : "bg-white"}`}
                              style={{ fontFamily: "Montserrat, sans-serif" }}
                            />
                          </FormControl>
