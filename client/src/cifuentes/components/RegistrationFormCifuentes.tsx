...
import TermsAndConditionsCifuentes from "./TermsAndConditionsCifuentes";
import PrivacyCifuentesPolicy from "./PrivacyPolicyCifuentes";
...
export default function RegistrationFormCifuentes({
  videoWatched,
  ...
  const { data: videoRequirementSetting } = useQuery({
    queryKey: ["/api/settings/cifuentes/video_requirement_enabled"],
    queryFn: async () => {
      const response = await apiRequest("GET", "/api/cifuentes/settings/video_requirement_enabled");
  ...
  const registerMutation = useMutation({
    mutationFn: async (data: FormData) => {
      const response = await apiRequest("POST", "/api/cifuentes/register", {
        ...data,
        videoWatched,
      });
      return response.json();
    ...
    onSuccess: (variables) => {
      ...
      queryClient.invalidateQueries({ queryKey: ["/api/cifuentes/stats"] });
      form.reset();
      ...
      <div className="container mx-auto px-4 relative z-10">
        <div className="max-w-2xl mx-auto">
          <div className="text-center mb-12">
            <div className="inline-block p-4 bg-gradient-to-r from-[#DEB406]  to-[#10155E] rounded-full mb-8 shadow-xl">
              <Trophy className="text-white" size={40} />
            </div>
            <h2
              className="text-3xl md:text-4xl font-black text-[#10155E] mb-4"
          ...
          {/* Registration Form */}
          <div className="bg-gradient-to-r from-[#DEB406]  to-[#10155E] rounded-2xl shadow-2xl p-2">
            <div className="bg-white rounded-xl p-8 shadow-inner relative">
              {/* Play Now badge */}
              <div className="absolute -top-4 left-1/2 transform -translate-x-1/2 bg-gradient-to-r from-[#DEB406]  to-[#10155E] text-white px-6 py-2 rounded-full shadow-lg">
                <span
              ...
              <Form {...form}>
                ...
                >
                  ...
                  <div className="relative">
                    {(!videoRequirementEnabled || videoWatched) && (
                      <div className="absolute -inset-1 bg-gradient-to-r from-[#DEB406]  to-[#10155E] rounded-lg blur opacity-30"></div>
                    )}
                    <Button
                      type="submit"
                      disabled={(videoRequirementEnabled && !videoWatched) || registerMutation.isPending}
                      className={`relative w-full bg-gradient-to-r from-[#DEB406]  to-[#10155E] hover:bg-gradient-to-r from-[#DEB406]  to-[#10155E] text-white font-black py-4 sm:py-5 md:py-6 px-4 sm:px-6 md:px-8 rounded-lg transform hover:scale-105 transition-all duration-300 shadow-2xl text-lg ${
                        (videoRequirementEnabled && !videoWatched)
  ...
  <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60 p-4">
    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[80vh] overflow-y-auto relative p-6">
      ...
      {/* Modal content */}
      {openModal === "terms" && <TermsAndConditionsCifuentes />}
      {openModal === "privacy" && <PrivacyCifuentesPolicy />}
    </div>
  </div>
)}
