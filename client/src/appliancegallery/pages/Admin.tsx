...
export default function AdminApplianceGallery() {
  ...
  const {
    ...
    queryKey: ["/api/appliancegallery/admin/registrations"],
    queryFn: async () => {
      const response = await apiRequest("GET", "/api/appliancegallery/admin/registrations");
      if (!response.ok) {
  ...
  const deleteRegistration = useMutation({
    mutationFn: async (id: string) => {
     const response = await apiRequest("DELETE", `/api/appliancegallery/admin/registrations/${id}`);
      if (!response.ok) {
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/appliancegallery/admin/registrations"] });
      toast({
        title: "Success",
  ...
  const bulkDeleteMutation = useMutation({
    mutationFn: async (ids: string []) => {
       const response = await apiRequest("POST", "/api/appliancegallery/admin/registrations/bulk-delete", { ids });
      if (!response.ok) {
